...
interface PlayerPanelProps {
  ...
  /** How many cards each player holds — 3 (default) or 4 */
  cardsPerPlayer?: number
}

function makeEmptyLockedBy(count: number): LockInfo[] {
  return Array.from({ length: count }, () => ({ lockerId: null, lockerName: null }))
}
...
function PlayerPanel({
  ...
  const lockInfos = lockedBy ?? makeEmptyLockedBy(cardsPerPlayer)
